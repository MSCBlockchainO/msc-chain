...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\mainnet_ddos_spam_test.ps1 -TargetBase https://wallet.mscblockexplorer.in -DurationSeconds 60 -Concurrency 16 -RequestsPerWorker 120
...
- Explorer and wallet public through separate HTTPS subdomains.
...
No cross-chain route may carry user funds until its route-specific contract,
observer/proof, signer-separation, accounting, testnet, audit, and operations
gates pass. The complete checklist and current trust-model boundary are in
`docs/BRIDGE_PRODUCTION_GATE.md`.

Configured bridge catalog entries remain inactive until their own evidence is
published. Mainnet chain readiness does not imply bridge route readiness.
...
- Explorer and wallet work over separate HTTPS subdomains.
