...
func (n *Node) setAcceptedProposalLocked(block Block, reason string, force bool) bool {
	...
	if currentKey != "" {
		...
		if prevBlock.ID == block.ID && proposalConflictsWithAcceptedLock(prevBlock, block) {
			keep, lockReason := n.proposalShouldStayLocked(prevBlock, prevVotes)
			if keep {
				commitVotes, required, _ := n.proposalHasSignedCommitQuorum(prevBlock)
				if lockReason == "local_signed_execution_quorum_locked" {
...
func (n *Node) rebroadcastCommitVoteForProposal(block Block, execHash string, txMerkle string, reason string) bool {
	...
	if ready, gateReason := n.validatorParticipationGateStatus(block.ID); !ready {
		if n.shouldLogLivenessReason(fmt.Sprintf("commit_rebroadcast_defer:%d:%s", block.ID, gateReason), 10*time.Second) {
			log.Printf("[COMMIT-REBROADCAST-DEFER] validator=%s height=%d reason=%s block=%s",
				ShortID(n.ID),
				block.ID,
				strings.TrimSpace(gateReason),
				ShortHash(block.BlockHash),
			)
		}
		return false
...
func (n *Node) executionCommitPrecondition(epoch uint64, leaderBlock Block) (int, int, bool, string) {
	...
	if commitVotes, commitRequired, committed := n.proposalHasSignedCommitQuorum(leaderBlock); committed {
		if commitRequired > 0 {
			required = commitRequired
		}
		return commitVotes, required, true, "signed_commit_quorum"
	}
...
func (n *Node) recoverSignedCommitQuorumAtCurrentHeight(reason string) bool {
	...
	n.execResultsMu.Lock()
	_ = n.setQuorumLockedProposalLocked(block, "signed_commit_recovery", executionVotes, required)
	n.execResultsMu.Unlock()
